...
if __name__ == '__main__':
    ...
    while itr<lbin_cosmo_1.size:
        ...
        loc_mat = np.zeros((nbins_cosmo,nbins_cosmo))
        for l1 in range(0,nbins_cosmo):
            for l2 in range(0,nbins_cosmo):
                loc_mat[l1,l2] = cov_g_cosmo[itr]
                itr+=1

        cov_g_mat_cosmo[zs[0],zs[1],zs[2],zs[3]] = loc_mat
...
#                for i4 in range(tomo_bins_cosmo):
...
#                        for l2 in range(nbins_cosmo):
    ...
    #k_in=d[:,0]; P_in=d[:,1]
    k_in,P_in=camb_pow(defaults.cosmology_cosmolike)
    ...
    sp = shear_power(k_in,C,z_fine,l_mids,fsky_cosmo,pmodel='halofit',P_in=P_in,ps=n_z,params=len_params)
    ...
    for z1 in range(0,tomo_bins_cosmo): 
        ...
        for z2 in range(0,tomo_bins_cosmo): 
            for z3 in range(0,tomo_bins_cosmo): 
                for z4 in range(0,tomo_bins_cosmo): 
                    ns = np.array([0.,0.,0.,0.])
                    if z1==z3:
