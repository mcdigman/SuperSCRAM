...
#	np.save('xiao.dat', np.column_stack((q,P)) )
...
#	from scipy.integrate import trapz 
	r=np.logspace(np.log10(1e-3),np.log10(20),500)
...
# 	print .5*(trapz(F_2(r3,1),r3) - trapz(F_2(r2,1),r2) )
	...
	for a in range(k.size):
		...
		r=np.logspace(np.log10(q_min/k[a]), np.log10(q_max/k[a]), N_q) 
		#r=np.logspace(np.log10(q_min), np.log10(q_max), N_q) 
		# id=np.where( (r[:] >=.95) & (r[:] <=1.05) )
# 		id=id[0]
# 		n=id.size
# 		r2=np.linspace(.85,1.15,n)
# 		r[id]=r2
# 		print 'n', n 
# 		
		...
		for b in range(N_q):
			...
			P_mu=P_L(k[a]*psi)
			P_mu[id]=0.0
			I_22=romb(P_mu*F_2(r[b],mu)**2, mu) 
			I_S2=trapz(P_mu*S_2(r[b],mu)**2, mu) 
			
			I_r_22[b]=r[b]**3*P_L(r[b]*k[a])*I_22
			I_r_S2[b]=r[b]**2*P_L(r[b]*k[a])*I_S2
			
		P_22[a]=romb(I_r_22,np.log10(r)) 
