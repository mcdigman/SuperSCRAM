...
#    print("radius for n_x="+str(n_x)+" 1000 deg^2="+str(radius))
    ...
    if do_lsst_test:
        ...
        if do_plot:
            ...
            #plot_reconstruction(m,ax,pixels,geo6.union_contains*1.)
            #hp.mollview(geo6.union_contains*1.,title='Ground Truth',fig=fig,hold=True,cmap='Greys')
            plot_reconstruction(geo6.union_contains*1.,'Ground Truth',fig)
            ...
            #plot_reconstruction(m,ax,pixels,reconstruct6)
            #geo4.union_mask.draw(m,color='blue')
            abs_error = np.abs(reconstruct6-geo6.union_contains*1.)
